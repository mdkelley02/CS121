...
public class MASHGame {
	...
	public static void main(String[] args) {
		...
		for (int item = 0; item < maleSpouseList.size(); item++) {
			System.out.print(maleSpouseList.get(item) + " ");
		}
		System.out.println();
		
		/* TODO: 8. Print the items in the occupationList using the ArrayList iterator and a while loop */
		Iterator<String> occupationListIterator = occupationList.iterator();
		while(occupationListIterator.hasNext()) {
			System.out.print(occupationListIterator.next() + ", ");
		}
		System.out.println();
		...
		String spouse = coin ? maleSpouseList.get(rand.nextInt(maleSpouseList.size())): femaleSpouseList.get(rand.nextInt(femaleSpouseList.size())) ; 
